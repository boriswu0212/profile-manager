...
- `internal/updater` — self-update: fetches the latest GitHub release, verifies sha256 checksum against `checksums.txt`, extracts the binary from the tar.gz archive, and atomically replaces the running binary via same-filesystem `os.Rename`.
